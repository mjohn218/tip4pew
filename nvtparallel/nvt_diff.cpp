...
int main(int argc, char* argv[])
{
  ...
  */
      ...
      if(t%100000==0)
	{
	  ...
	  if(rank==0)
	      ...
	      for(i=0;i<cons.N;i++)
		{
	      ...
	      pout.close();
	      sprintf(filet, "therm_equil.%d",t/100000);
	      thermfile.open(filet);
	      thermfile <<th.Eta[0]<<' '<<th.Eta[1]<<' '<<th.Eta[2]<<' '<<th.Eta[3]<<' '<<th.Eta[4]<<" #therm pos and vel "<<endl;
  ...
  if(rank==0)
    {
      thermfile.open("Equil_therm.out");
      thermfile <<th.Eta[0]<<' '<<th.Eta[1]<<' '<<th.Eta[2]<<' '<<th.Eta[3]<<' '<<th.Eta[4]<<" #therm pos and vel "<<endl;
      thermfile <<th.Veta[0]<<' '<<th.Veta[1]<<' '<<th.Veta[2]<<' '<<th.Veta[3]<<' '<<th.Veta[4]<<endl;
      thermfile.close();
    }
      ...
      if(t%parms.allsave==0)
	{
	  ...
	  if(rank==0)
	      ...
	      for(i=0;i<cons.N;i++)
		{
	      ...
	      pout.close();
	      sprintf(filet, "therm_all.%d",t/parms.allsave);
	      thermfile.open(filet);
	      thermfile <<th.Eta[0]<<' '<<th.Eta[1]<<' '<<th.Eta[2]<<' '<<th.Eta[3]<<' '<<th.Eta[4]<<" #therm pos and vel "<<endl;
	      thermfile <<th.Veta[0]<<' '<<th.Veta[1]<<' '<<th.Veta[2]<<' '<<th.Veta[3]<<' '<<th.Veta[4]<<endl;
	      thermfile.close();
  ...
  if(rank==0)
    {
      ...
      afile <<a.Pavg/(t*1.0)<<endl;
      afile <<a.Tavg/(t*1.0)<<endl;
      afile <<a.Epavg/(t*1.0)<<endl;      
      afile <<a.P2avg/(t*1.0)<<endl;
      afile <<a.Ep2avg/(t*1.0)<<endl;
