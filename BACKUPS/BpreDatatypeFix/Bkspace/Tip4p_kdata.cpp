...
void NVEintegrateMotion(Ensemble &NVE, Constants cons, Parameters parms, WaterSend *mols, WaterSend *local_mols, MPI::Datatype Watertype, int t, Kspace *kvector, MPI::Datatype Kv);
void mKWALD(Ensemble &NNN, Constants cons, Parameters parms, WaterSend *mols, WaterSend *local_mols, Kspace *kvector, MPI::Datatype Kv);
void mcalcEForce(Ensemble &NNN, Constants cons, Parameters parms, WaterSend *mols, WaterSend *local_mols, Kspace *kvector, MPI::Datatype Kv);
...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  MPI::Datatype typek[2]={MPI_DOUBLE, MPI_DOUBLE};
  MPI::Aint dispk[2];
  int blockk[2]={2443,2443};
  /*Make Kvector object*/
  Kspace * kvector=new Kspace;
  dispk[0]=0;
  dispk[1]=19544;
  Kv=Kv.Create_struct(2, blockk, dispk, typek);
  size=Kv.Get_size();
  cout << " Size of K is: "<< size <<endl;
  Kv.Commit();
  ...
  */
  ...
  mcalcEForce(NVE, cons, parms, mols, local_mols, kvector, Kv);
  ...
  */
  ...
  for(t=0;t<cons.Tsteps;t++)
	...
	inside NVEintegrateMotion is an AllGather MPI call.
      ...
      NVEintegrateMotion(NVE,cons,parms, mols, local_mols, Watertype, t, kvector, Kv);
...
void mcalcEForce(Ensemble &NNN, Constants cons, Parameters parms, WaterSend *mols, WaterSend *local_mols, Kspace *kvector, MPI::Datatype Kv)
{
  ...
  */
  ...
  mKWALD(NNN, cons, parms, mols, local_mols, kvector, Kv);
