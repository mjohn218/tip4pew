      ...
      for(ky=-10;ky<11;ky++)
	{
	  ...
	  for(kz=-10;kz<11;kz++)
	      ...
	      if(Ksq!=0 && Ksq <= Kscut)
		{
	      ...
	      zi++;
	      cout << "after zi++ should be -313: "<<Kreal[0]<<endl;
	    }
	  yi++;
	  cout << "after yi++ should be -313: "<<Kreal[0]<<endl;
	}
    }
	      ...
	      if(Ksq!=0 && Ksq <= Kscut)
		{
		  ...
		  for(i=0;i<cons.NperP;i++)
		      ...
		      for(j=0;j<3;j++)
			{
			  ...
			  NNN.Fy[i][j+1]+=Fyik;
			  NNN.Fz[i][j+1]+=Fzik;
			  Xp=mols[i].x[j+1]-NNN.XCOM[i];
			  Yp=mols[i].y[j+1]-NNN.YCOM[i];
			  Zp=mols[i].z[j+1]-NNN.ZCOM[i];
			  VIRMxx+=Xp*Fxik;
			  VIRMyy+=Yp*Fyik;
			  VIRMzz+=Zp*Fzik;
			}
