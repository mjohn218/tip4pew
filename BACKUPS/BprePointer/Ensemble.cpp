...
void Ensemble::RigidMol(Parameters &parms, Constants cons, WaterSend * mols, DiffSend *opos)
{
...
void Ensemble::printCoords(char *filename, Constants cons, WaterSend *mols, int t)
{
  int i;
...
void Ensemble::printVel(char *filename, Constants cons, int t)
{
  int i;
  ofstream outFile(filename);
...
void Ensemble::RWALD(Constants cons, Parameters parms, WaterSend *mols, WaterSend *local_mols)
{
  ...
  */
  ...
  Energyv[0]=E0; //Energy oxygen
  Energyv[1]=EC; //Coulomb charges
  Energyv[2]=RealVir_LJ; // Virial from real space
...
void Ensemble::Maxwell(Constants cons, Parameters parms, WaterSend *vel)
{
  int i;
...
void Ensemble::RattlePos(Constants cons, Parameters parms, WaterSend *local_mols, DiffSend *myopos)
{
  ...
  for(i=0;i<cons.NperP;i++)
      ...
      for(j=0;j<cons.NUMPRIM;j++)
	{
	  ...
	  myopos[i].x[j]+=cons.dt*vx[i][j];
	  myopos[i].y[j]+=cons.dt*vy[i][j];
	  myopos[i].z[j]+=cons.dt*vz[i][j];
	}
...
void Ensemble::VelocityNormal(Constants cons, Parameters parms, WaterSend *mols, WaterSend * vel)
{
  int i;
  double * r01=new double[3];
  double * r02=new double[3];
...
//    ELRCint and PLRCint and VirTotal set here.
  ...
  VirTotal=RealVir_LJ+KVirial;
  PLRCint=PLRC/parms.Volume;
  
  Pint=(kineCOM+PLRCint+VirTotal)/parms.pconstant;
  
  cout <<"RealVir_LJ: "<<RealVir_LJ <<"KVir_LJ: "<<KVirial<<endl;
  cout <<"Pint: "<< Pint<<endl;
