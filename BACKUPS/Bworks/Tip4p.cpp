...
void NVEintegrateMotion(Ensemble &NVE, Constants cons, Parameters parms, WaterSend *mols, WaterSend *local_mols, MPI::Datatype Watertype, int t, double *Kreal, double *Kimag, double *SKreal, double *SKimag, Kindex *kxyz);
...
void Ksetup2(Constants cons, Kindex *kxyz);
...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  cout << "IN main: klength: " <<cons.Kleng <<endl;
  if(rank==0)
    {
      ofstream kvfile("mykIndexes.out");

      for(i=0;i<cons.Kleng;i++)
	{
	  kvfile << kxyz[i].kx <<' '<< kxyz[i].ky<<' '<<kxyz[i].kz<<endl;
	}
      kvfile.close();
    }
  /*
  ...
  */
  ...
  for(t=0;t<cons.Tsteps;t++)
	...
	inside NVEintegrateMotion is an AllGather MPI call.
      ...
      NVEintegrateMotion(NVE,cons,parms, mols, local_mols, Watertype, t, Kreal,  Kimag, SKreal, SKimag, kxyz);
  
      if(rank==0)
	{
	  char * fileforc="FinalForce.out";
	  NVE.writeAllForce(fileforc, cons);
	}
...
void mcalcEForce(Ensemble &NNN, Constants cons, Parameters parms, WaterSend *mols, WaterSend *local_mols, double *Kreal, double *Kimag, double *SKreal, double *SKimag, Kindex *kxyz)
{
  ...
  */
  if(cons.rank==0)
    {
      char * rwaldpos="rwaldPos.out";
      NNN.printAllMyCoords(rwaldpos, cons, local_mols);
    }
...
void Ksetup1(Constants &cons)
{
  ...
  cout << "Size of K vector: " << lengthK <<endl;
  cons.Kleng=lengthK;
