...
void NVEintegrateMotion(Ensemble &NVE, Constants cons, Parameters parms, WaterSend *mols, WaterSend *local_mols, MPI::Datatype Watertype, int t, double *Kreal, double *Kimag, double *SKreal, double *SKimag, Kindex *kxyz, DiffSend *myopos, DiffSend *vel, WaterSend *force, Center *com, Table *terf, double *Veck, kspace *xr, kspace *xi, kspace *yr, kspace *yi, kspace *zr, kspace *zi, kspace *ekr, kspace *eki, double *Mat, double *Minv);
...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  /*Read in the coordinates into object mols*/
  ...
  cout << " CHECK ENSEMBLE.H, IF nperp IS NOT SET TO CORRECT N/PROCESSORS: "<< cons.NperP;
  cout<<", FIX AND RECOMPILE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n"<<endl;
  ...
  */  
  ...
  mcalcEForce(NVE, cons, parms, mols, local_mols, Kreal, Kimag, SKreal, SKimag, kxyz, force, com, terf, Veck, xr, xi, yr, yi, zr, zi, ekr, eki);
  ...
  */
  ...
  xx=local_mols[250].x[0]-local_mols[250].x[1];
  yy=local_mols[250].y[0]-local_mols[250].y[1];
  zz=local_mols[250].z[0]-local_mols[250].z[1];
  ...
  xv=myvel[250].x[0]-myvel[250].x[1];
  yv=myvel[250].y[0]-myvel[250].y[1];
  zv=myvel[250].z[0]-myvel[250].z[1];
  ...
  */
  ...
  for(t=1;t<Teqp1;t++)
	...
	inside NVEintegrateMotion is an AllGather MPI call.
      ...
      NVEintegrateMotion(NVE,cons,parms, mols, local_mols, Watertype, t, Kreal,  Kimag, SKreal, SKimag, kxyz, myopos, myvel, force, com, terf, Veck, xr, xi, yr, yi, zr, zi, ekr, eki, Mat, Minv);
