...
{
  //if(vec>=0)  return -(int)(vec/boxlen+.5);
  //else  return (int)(.5-vec/boxlen);
  return -round(vec/boxlen);
}
...
and establish teh position of the imaginary 4th 'atom' of water*/
  ...
  for(i=0;i<cons.N;i++)
      ...
      if(parms.BOXLOLD>2.0)
	{
      ...
      XHH+=parms.BOXLENGTH*round(XHH, parms.BOXLENGTH);
      YHH+=parms.BOXLENGTH*round(YHH, parms.BOXLENGTH);
      ZHH+=parms.BOXLENGTH*round(ZHH, parms.BOXLENGTH);
...
void RWALD(Constants cons, Parameters parms, WaterSend *mols, WaterSend *local_mols, WaterSend *force, Center *com, Ensemble &nnn, Table *terf)
{
  ...
  double r02;
  double r2, r6, r12;
  /*Return these to main Energy Oxygens (LJ) and Energy Coulomb (H, H, M)*/
  nnn.E0=0.0;
  nnn.EC=0.0;
	   ...
	   if( j!=(i+cons.rank*cons.NperP))
	     {
	       ...
	       rX0=rX0+parms.BOXLENGTH*round(rX0,parms.BOXLENGTH);
	       rY0=rY0+parms.BOXLENGTH*round(rY0,parms.BOXLENGTH);
	       rZ0=rZ0+parms.BOXLENGTH*round(rZ0,parms.BOXLENGTH);
	       ...
	       if(r02<=cons.R_LOWER2)
		 {
		   ...
		   xCMnow+=parms.BOXLENGTH*round(xCMnow, parms.BOXLENGTH);
		   yCMnow+=parms.BOXLENGTH*round(yCMnow, parms.BOXLENGTH);
		   zCMnow+=parms.BOXLENGTH*round(zCMnow, parms.BOXLENGTH);
		   ...
		   for(k=1;k<cons.NSITE;k++)
		       ...
		       for(n=1;n<cons.NSITE;n++)
			 {
			   ...
			   RXnew+=parms.BOXLENGTH*round(RXnew,parms.BOXLENGTH);
			   RYnew+=parms.BOXLENGTH*round(RYnew,parms.BOXLENGTH);
			   RZnew+=parms.BOXLENGTH*round(RZnew,parms.BOXLENGTH);
...
void Maxwell(Constants cons, Parameters parms, DiffSend *vel)
{
  ...
  for(i=0;i<cons.N;i++)
      ...
      for(j=0;j<cons.NUMPRIM;j++)
	{
	  ...
	  sumx+=parms.AMass[j]*vel[i].x[j];
	  sumy+=parms.AMass[j]*vel[i].y[j];
	  sumz+=parms.AMass[j]*vel[i].z[j];
	  amsum+=parms.AMass[j];
	}
    }
  sumx=sumx/amsum;
  ...
  for(i=0;i<cons.NperP;i++)
    {
      ...
      r01[0]+=parms.BOXLENGTH*round(r01[0], parms.BOXLENGTH);
      r01[1]+=parms.BOXLENGTH*round(r01[1], parms.BOXLENGTH);
      r01[2]+=parms.BOXLENGTH*round(r01[2], parms.BOXLENGTH);
      
      /*distance O to H2*/
      r02[0]=local_mols[i].x[0]-local_mols[i].x[2];
      r02[1]=local_mols[i].y[0]-local_mols[i].y[2];
      r02[2]=local_mols[i].z[0]-local_mols[i].z[2];
      r02[0]+=parms.BOXLENGTH*round(r02[0], parms.BOXLENGTH);
      r02[1]+=parms.BOXLENGTH*round(r02[1], parms.BOXLENGTH);
      r02[2]+=parms.BOXLENGTH*round(r02[2], parms.BOXLENGTH);

      /*ditsnce H to H*/
      r12[0]=local_mols[i].x[1]-local_mols[i].x[2];
      r12[1]=local_mols[i].y[1]-local_mols[i].y[2];
      r12[2]=local_mols[i].z[1]-local_mols[i].z[2];
      r12[0]+=parms.BOXLENGTH*round(r12[0], parms.BOXLENGTH);
      r12[1]+=parms.BOXLENGTH*round(r12[1], parms.BOXLENGTH);
      r12[2]+=parms.BOXLENGTH*round(r12[2], parms.BOXLENGTH);
      ...
      while(it<parms.iterRat)
	{
	  ...
	  s01[1]=sy0-sy1;
	  s01[2]=sz0-sz1;
	  s01[0]+=parms.BOXLENGTH*round(s01[0], parms.BOXLENGTH);
	  s01[1]+=parms.BOXLENGTH*round(s01[1], parms.BOXLENGTH);
	  s01[2]+=parms.BOXLENGTH*round(s01[2], parms.BOXLENGTH);
	  
	  s02[0]=sx0-sx2;
	  s02[1]=sy0-sy2;
	  s02[2]=sz0-sz2;
	  s02[0]+=parms.BOXLENGTH*round(s02[0], parms.BOXLENGTH);
	  s02[1]+=parms.BOXLENGTH*round(s02[1], parms.BOXLENGTH);
	  s02[2]+=parms.BOXLENGTH*round(s02[2], parms.BOXLENGTH);
	  
	  s12[0]=sx1-sx2;
	  s12[1]=sy1-sy2;
	  s12[2]=sz1-sz2;
	  s12[0]+=parms.BOXLENGTH*round(s12[0], parms.BOXLENGTH);
	  s12[1]+=parms.BOXLENGTH*round(s12[1], parms.BOXLENGTH);
	  s12[2]+=parms.BOXLENGTH*round(s12[2], parms.BOXLENGTH);
      ...
      for(j=0;j<cons.NUMPRIM;j++)
	{
      ...
      xpbc=parms.BOXLENGTH*round(local_mols[i].x[0], parms.BOXLENGTH);
      ypbc=parms.BOXLENGTH*round(local_mols[i].y[0], parms.BOXLENGTH);
      zpbc=parms.BOXLENGTH*round(local_mols[i].z[0], parms.BOXLENGTH);
  ...
  for(i=0;i<cons.N;i++)
    {
      ...
      vel[i].x[0]-=m1*L1*r01[0]-m1*L2*r02[0];
      vel[i].y[0]-=m1*L1*r01[1]-m1*L2*r02[1];
      vel[i].z[0]-=m1*L1*r01[2]-m1*L2*r02[2];
      vel[i].x[1]+=m2*L1*r01[0]-m2*L3*r12[0];
      vel[i].y[1]+=m2*L1*r01[1]-m2*L3*r12[1];
      vel[i].z[1]+=m2*L1*r01[2]-m2*L3*r12[2];
      vel[i].x[2]+=m3*L2*r02[0]+m3*L3*r12[0];
      vel[i].y[2]+=m3*L2*r02[1]+m3*L3*r12[1];
      vel[i].z[2]+=m3*L2*r02[2]+m3*L3*r12[2];
