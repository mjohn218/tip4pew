...
void NVTintegrateMotion(Ensemble &NVE, Constants cons, Parameters parms, DiffSend *mols, DiffSend *local_mols, MPI::Datatype Difftype, int t, DiffSend *myopos, DiffSend *vel, DiffSend *force, Center *com, Table *terf, double *Mat, double *Minv, Thermostat &th);

//void mKWALD(Ensemble &NNN, Constants cons, Parameters parms, DiffSend *mols, DiffSend *local_mols, double *Kreal, double *Kimag, double *SKreal, double *SKimag, Kindex *kxyz, DiffSend *force, Center *com, double *Veck, kspace *xr, kspace *xi, kspace *yr, kspace *yi, kspace *zr, kspace *zi, kspace *ekr, kspace *eki);

void mcalcEForce(Ensemble &NNN, Constants cons, Parameters parms, DiffSend *mols, DiffSend *local_mols, DiffSend *force, Center *com, Table *terf);
...
int main(int argc, char* argv[])
{
  ...
  /*DiffSend and DiffSend Classes defined in Ensemble.h*/
  DiffSend * mols= new DiffSend[cons.N];
  DiffSend * local_mols = new DiffSend[cons.NperP];
  ...
  Center *com=new Center[cons.N];
   ...
   coordinates go into DiffSend mols object.Then split them up into a local
   set
  */
  ...
  */
  ...
  DiffSend * force=new DiffSend[cons.NperP];
...
//   kspace * eki=new kspace[cons.Kleng];
  ...
  */  
  ...
  mcalcEForce(NVT, cons, parms, mols, local_mols, force, com, terf);
  ...
  */
  ...
  char * ideal="ideal.out";
  checkIdeal(ideal, cons, parms,mols);
  ...
  */
  ...
  for(t=1;t<Teqp1;t++)
	...
	inside NVTintegrateMotion is an AllGather MPI call.
      ...
      NVTintegrateMotion(NVT,cons,parms, mols, local_mols, Difftype, t, myopos, myvel, force, com, terf,Mat, Minv, th);
...
void NVTintegrateMotion(Ensemble &NVT, Constants cons, Parameters parms, DiffSend *mols, DiffSend *local_mols, MPI::Datatype Difftype, int t, DiffSend *myopos, DiffSend *vel, DiffSend *force, Center *com, Table *terf, double *Mat, double *Minv, Thermostat &th)
{
  /*NVT Velocity Verlet*/
  int i;
  double Ekint;
  ...
  */
  ...
  MPI::COMM_WORLD.Allgather(local_mols, cons.NperP, Difftype, mols, cons.NperP, Difftype);
...
/*Sum the energies across processors*/
  ...
  if(cons.rank==0)
      ...
      for(i=1;i<cons.MC;i++)
	{
      ...
      cout << "E0 "<< Efinish[0]<<" EC " <<Efinish[1] << " VK "<< NNN.VK <<" Vex " <<Efinish[3]<<" Vs "<<NNN.Vs<<endl;
      ...
      cout <<endl;
      cout << "RealVir: "<< Efinish[2]<<" KVirial "<<NNN.KVirial<<" PLRCint: "<<NNN.PLRCint<<endl;
      cout <<"KineCOM: "<<Efinish[5]<<" Pconstant:1/(3V atm) "<<parms.pconstant<<endl;
