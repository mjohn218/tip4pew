...
void readNVTParms(char* filename, Constants &cons, Parameters &parms, Thermostat&th)
{
  ...
  PARMFILE >> cons.NYS;
  PARMFILE.ignore(400, '\n');
  PARMFILE >> th.Eta[0]>>th.Eta[1]>>th.Eta[2]>>th.Eta[3]>>th.Eta[4];
  PARMFILE.ignore(400, '\n');
  PARMFILE >> th.Veta[0]>>th.Veta[1]>>th.Veta[2]>>th.Veta[3]>>th.Veta[4];
  PARMFILE.ignore(400, '\n');
  PARMFILE >> th.taup;
  PARMFILE.ignore(400, '\n');
  PARMFILE.close();
...
void InitialParms(Parameters &parms, Constants &cons, Averages &avg, Ensemble &nnn)
{
  ...
  parms.THETAH=109.471;
  parms.ROH=1.0;
  parms.RHH=1.6329919;//1.6332831103;
...
and establish teh position of the imaginary 4th 'atom' of water*/
 ...
 //      //Now move the hydrogen farther away!
//       mols[i].x[1]=mols[i].x[0]+XH10/.9572;
//       mols[i].y[1]=mols[i].y[0]+YH10/.9572;
//       mols[i].z[1]=mols[i].z[0]+ZH10/.9572;
      ...
      for(j=0;j<cons.N;j++)
	{
	   if( j!=(i+cons.rank*cons.NperP))
	       ...
	       if(r02<=cons.R_LOWER2)
		 {
		   ...
		   //potential is now a  coulomb, hence erfc, no LJ
		   for(k=0;k<cons.NSITE;k++)
		     {
		       for(n=0;n<cons.NSITE;n++)
			 {
...
void RattleVel(Constants cons, Parameters parms, DiffSend *local_mols, DiffSend *vel, double *Mat, double *Minv)
{
  ...
  /*Same Matrix every time step*/
 //  1.30418920749822  -0.33745574137607   0.47279638566401
//   -0.33745574137607   1.30418920749822  -0.47279638566401
//    0.47279638566401  -0.47279638566401   0.62756427497614

  
//   Minv[0]=1.30418920749822;
//   Minv[1]=-0.33745574137607;
//   Minv[2]=0.47279638566401;
//   Minv[3]=-0.33745574137607;
//   Minv[4]=1.30418920749822;
//   Minv[5]=-0.47279638566401;
//   Minv[6]=0.47279638566401;
//   Minv[7]=-0.47279638566401;
//   Minv[8]=0.62756427497614;
...
//       cout << Minv[3]<<' '<<Minv[4]<<' '<<Minv[5]<<endl;
//       cout << Minv[6]<<' '<<Minv[7]<<' '<<Minv[8]<<endl;
...
{
  /*From Martyna et al J. Mol Phys 1996*/

  int const MCEND=cons.MC-1; //because in cpp end of array is length-1.
  int inc, jnys, imc;
  double AKIN;
  double scale=1.0;
  double AA;
 
 /*Nose Hoover part of integration, i.e. Liouville iL_NHC.
   */
  //Get the toal kinetic energy
  
  AKIN=Ekin*2.0*418.4;//KineticA(c, cons);

  //Update the forces
  therm.Gt[0]=(AKIN-therm.gnkt)/therm.QMass[0];

  //start the multiple evolution operators
  for(inc=0;inc<cons.NTC;inc++)
    {
      for(jnys=0;jnys<cons.NYS;jnys++)
	{

	  //update thermostat velocities
	  therm.Veta[MCEND]=therm.Veta[MCEND]+therm.Gt[MCEND]*therm.wdti4[jnys];
	  for(imc=0;imc<(cons.MC-1);imc++)
	    {
	      AA=exp(-therm.wdti8[jnys]*therm.Veta[MCEND-imc]);
	      therm.Veta[MCEND-imc-1]=therm.Veta[MCEND-imc-1]*AA*AA
		+therm.wdti4[jnys]*therm.Gt[MCEND-imc-1]*AA;
	    }

	  //update particle velocities scalar
	  AA=exp(-therm.wdti2[jnys]*therm.Veta[0]);
	  scale=scale*AA;
	  
  	  //update the forces
	  therm.Gt[0]=(scale*scale*AKIN-therm.gnkt)/therm.QMass[0];
	  
	  //update the thermostat positions
	  for(imc=0;imc<cons.MC;imc++)
	    {
	      therm.Eta[imc]=therm.Eta[imc]+therm.Veta[imc]*therm.wdti2[jnys];
	    }

	  //update the thermostat velocities
	  for(imc=0;imc<(cons.MC-1);imc++)
	    {
	      AA=exp(-therm.wdti8[jnys]*therm.Veta[imc+1]);
	      therm.Veta[imc]=therm.Veta[imc]*AA*AA
		+therm.wdti4[jnys]*therm.Gt[imc]*AA;
	      therm.Gt[imc+1]=(therm.QMass[imc]*therm.Veta[imc]*therm.Veta[imc]-therm.gkt)/therm.QMass[imc+1];
	    
	    }
	  therm.Veta[MCEND]=therm.Veta[MCEND]+therm.Gt[MCEND]*therm.wdti4[jnys];
	}//end of loop over jnys
    }//end of loop over inc
...
void setThermostat(Thermostat &therm, Constants cons)
{
  int i;
  double w[5];
  therm.gkt=cons.BOLTZK*cons.Temp;
  therm.gnkt=cons.KbDF*cons.Temp;
  therm.QMass[0]=therm.gnkt*therm.taup*therm.taup;
  for(i=1;i<cons.MC;i++)
    {
      therm.QMass[i]=therm.gkt*therm.taup*therm.taup;
    }
  ...
  w[3]=w[0];
  w[4]=w[0];
  /*Gt[0] set in first NHCINT*/
  for(i=0;i<cons.MC-1;i++)
    {
      therm.Gt[i+1]=(therm.QMass[i]*therm.Veta[i]*therm.Veta[i]-therm.gkt)/therm.QMass[i+1];
    }
  for(i=0;i<cons.NYS;i++)
    {
      therm.wdti2[i]=(w[i]*cons.dt)/(2.0*cons.NTC);
      therm.wdti4[i]=0.5*therm.wdti2[i];
      therm.wdti8[i]=0.5*therm.wdti4[i];
    }
  therm.KE=0.0;
  therm.PE=0.0;
  therm.TE=0.0;
